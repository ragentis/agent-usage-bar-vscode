...
<img src="assets/screenshot.png" width="897" alt="The Codex and Claude items in the VS Code status bar, with both tooltips open above them: the plan name, then a filled bar, reset time and pace for each window, the thirty-day daily activity strip, and links to refresh or open the settings.">
...
The weekly bar also carries a mark standing where that elapsed percentage falls, so a glance says whether spending is ahead of the week or behind it. The mark is left off where it would say nothing: under the bar's rounded ends, or right beside the edge of the fill, which is the same comparison.

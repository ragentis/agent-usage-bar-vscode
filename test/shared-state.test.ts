...
const snapshot: UsageSnapshot = {
  windows: [
    ...
    { kind: "weekly", usedPercent: 41, resetsAt: null, windowMinutes: null, label: null },
    { kind: "weekly", usedPercent: 62, resetsAt: null, windowMinutes: null, label: "Fable" },
  ],
  plan: "plus",
  blocked: null,
  credits: "3 reset credits",
  creditsExpireAt: new Date("2026-08-12T17:31:56Z"),
        ...
        windows: [
          {
            ...
            windowMinutes: 300,
            // An entry written before scopes existed carries no label, and reads back without one.
            label: null,
          },
          {
            kind: "weekly",
...
test("what this window publishes is the shape v1 names", async () => {
  ...
  await shared.publish("claude", {
    ...
    view: {
      snapshot: {
        ...
        plan: "pro",
        blocked: null,
        credits: "1 reset credit",
        creditsExpireAt: new Date(1_754_600_000_000),
        fetchedAt: new Date(1_754_000_000_500),
        source: "claude-account-api",
      },
      message: null,
  ...
  expect(store.get("sharedUsage.v1.claude")).toMatchObject({
    ...
    snapshot: {
      ...
      blocked: null,
      credits: "1 reset credit",
      creditsExpireAt: 1_754_600_000_000,
      fetchedAt: 1_754_000_000_500,
      source: "claude-account-api",
    },
  });

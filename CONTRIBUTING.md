...
| `font` | Regenerates the day glyphs, the weekly mark and the tooltip variants of the provider marks, their manifest entries, and the icon font, through the Fontello API. On demand only; see **Day bars and the weekly mark**. |
...
The font contains a regular and a tooltip variant of each mark. The status bar centers the regular glyph with flexbox. Tooltips draw a theme icon at the size of the surrounding text and align it by baseline, which places these custom marks about one pixel too low beside the title, and trusted Markdown keeps no CSS declaration that could size or move an icon. So tooltips use dedicated `-hover` glyphs, and those are generated by `npm run font` from the hand-drawn marks: the outline scaled up about its centre, so the title reads behind a mark larger than its type, lifted by 39 font units, which at 1,000 units per em and 13px is about half a CSS pixel and grid fitting renders as one, and given an advance that grows with the scale plus a gap before the title. The numbers are `HOVER` in [build-font.mjs](scripts/build-font.mjs); verify alignment visually against a codicon, and at more than one display scale.
...
That regenerates the bars in `assets/fontello-config.json`, posts the whole configuration to the Fontello API, writes the returned font to `assets/agent-usage-bar.woff`, and rewrites the day and mark icons in `package.json`. The provider marks pass through untouched, in the configuration and in the manifest; the script replaces only glyphs named `day-*` and `mark*`, and rebuilds the `-hover` variants from the marks without touching their manifest entries. It runs on demand, never as part of a build, and everything it writes is committed.

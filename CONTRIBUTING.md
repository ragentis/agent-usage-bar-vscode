...
- The release pull request is opened by the default `GITHUB_TOKEN`, so GitHub may require a maintainer to click **Approve workflows to run** before its checks start. Phase 2 still verifies the exact release commit after merge. `main` does not require status checks, so this approval is not a release prerequisite.
